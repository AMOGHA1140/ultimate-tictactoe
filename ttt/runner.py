...
# state = ttt.State(x_bits=1407413439513188310779185, o_bits=25977361930701009585828, valid_mini_board=9)
...
def display_board():
    ...
    if state.valid_action!=9:
        R_valid, C_valid = state.valid_action//3, state.valid_action%3
    else:
        R_valid, C_valid = -1, -1
        ...
        for C in range(3):
            ...
            for r in range(3):
                ...
                for c in range(3):
                    ...
                    elif C_valid == -1 and not ttt.mini_board_terminal(state[R, C]):
                        pygame.draw.rect(screen, 'yellow', rect, 3)
                    else:
                        pygame.draw.rect(screen, 'white', rect, 3)
                    

                    if state[R, C, r, c] != ttt.EMPTY:
                        colour = 'red' if state[R, C, r, c]==ttt.X else 'blue'
                        move = move_font.render(state[R, C, r, c], True, colour)
        ...
        if click==1 and not game_over:
            ...
            for R in range(3):
                for C in range(3):
                    for r in range(3):
                        for c in range(3):
                            if tiles[R][C][r][c].collidepoint(mouse_position) and ttt.legal_action(state, (R, C, r, c)):
                                # print(f"waw-before-{state.valid_action}")
                                state = ttt.result(state, (R, C, r, c))
                                # print(f"waw-after-{state.valid_action}")
